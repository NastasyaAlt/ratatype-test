$(function () {
  ...
  TypingTrainer.prototype.populateLectionBuffer =  function () {
      if (/random-/g.test(this.config.lesson)) {
        while (this.lection
...
*/
  ...
  TypingTrainer.prototype.generateLection = function () {
      this.clearLection();
      this.populateLectionBuffer();
  };

  TypingTrainer.prototype.reloadLection = function (type) {
      this.config.lesson = type;
      this.generateLection();
  };

  TypingTrainer.prototype.tryRemove = function (input) {
      ...
      if (this.score.lastCheckLength <
        input.length) {
        this.score.misses += input.length - this.score.lastCheckLength;
  ...
  $('body').on('tt.lection_update',function () {
        ...
        while (i < tt.lection.length) {
          alpha = (tt.lection.length - i) /tt.lection.length;
          text +='<span>';
          if (i == 0 && tt.lection[
            i] == " ") {
            text += ' '
